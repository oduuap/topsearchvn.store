...
const LOCATIONS = {
  ...
  hcm: {
    ...
    serpapi: "Ho Chi Minh City, Vietnam",
    gl: "vn",
  },
...
async function getDebugBrowser() {
  ...
  try {
    ...
    if (!_debugBrowser) {
      _debugBrowser = await puppeteer.connect({
...
app.post("/api/launch-chrome", (req, res) => {
  ...
  if (!chromePath)
    return res.status(500).json({ error: "Không tìm thấy Chrome" });
...
async function crawlGoogle(
  ...
  if (SCRAPER_KEY) {
    ...
    organic_results = organic_results.map((r, i) => ({
      ...r,
      position: i + 1,
    }));
  ...
  if (isDebug && debugBrowser) {
    ...
    if (!chromePath)
      throw new Error(
        "VPS chưa cài Chromium. Chạy: sudo apt install -y chromium-browser",
      );
  ...
  try {
        ...
        for (const a of document.querySelectorAll(
          ...
          for (let i = 0; i < 6 && node; i++, node = node.parentElement) {
            const spans = Array.from(
              node.querySelectorAll("span, div[data-snf]"),
            )
              .map((el) => el.innerText?.trim() || "")
              .filter(
                (t) => t.length > 40 && !title.includes(t.substring(0, 20)),
              );
            if (spans.length) {
              snippet = spans[0];
              break;
          ...
          for (let i = 0; i < 6 && pNode; i++, pNode = pNode.parentElement) {
            ...
            if (cite) {
              displayedLink = cite.innerText.trim();
              break;
            }
          }
          if (!displayedLink) {
            try {
              displayedLink = new URL(href).hostname;
            } catch {}
          }
      ...
      .waitForFunction(
        () =>
          document.querySelector(
            "#rso a[ping], #rso a[data-ved], #captcha-form",
          ) !== null,
        { timeout: 12000 },
      )
      .catch(() => {});
    ...
    return {
      ...
      organic_results,
      source: "crawl",
      mode: isDebug ? "chrome-debug" : "headless",
    };
  } finally {
    await page.close();
...
async function geocodeCoords(lat, lon) {
  try {
    ...
    if (cc === "vn") {
      const key = cityKeyFromName(city || state);
      if (key)
        return { key, label: city || state, raw: LOCATIONS[key].serpapi };
      return {
        key: null,
        label: city || state,
        raw: city ? `${city}, Vietnam` : "Vietnam",
      };
    }
...
async function fetchSerpAPI(keyword, locationKey = "hcm", num = 20) {
  ...
  if (wantNum >= 20 && organic_results.length >= 8) {
    try {
      ...
      const page2 = (d2.organic_results || [])
        .map(mapOrg)
        .filter((r) => !existingLinks.has(r.link))
...
function mergeResults(serpData, crawlData) {
  ...
  crawlOrg.forEach((r) => {
    try {
      crawlMap.set(new URL(r.link).hostname, r);
    } catch {}
  });
  ...
  const merged = serpOrg.map((r) => {
    let crawlHost;
    try {
      crawlHost = new URL(r.link).hostname;
    } catch {
      crawlHost = "";
    }
  ...
  const crawlOnly = crawlOrg
    .filter((r) => {
      try {
        return !serpLinks.has(new URL(r.link).hostname);
      } catch {
        return false;
      }
    })
...
app.get("/api/locations", (req, res) => {
  res.json(
    Object.entries(LOCATIONS).map(([key, val]) => ({ key, label: val.label })),
  );
});
...
app.get("/api/detect-location", async (req, res) => {
  try {
    ...
    const locationKey = cityKeyFromName(city) || "hcm";
...
app.get("/api/search", async (req, res) => {
  ...
  try {
    ...
    if (source === "serpapi" || source === "both") {
      try {
        serpData = await fetchSerpAPI(q, location, num);
      } catch (e) {
        errors.serpapi = e.message;
        console.error("SerpAPI err:", e.message);
      }
    }

    if (source === "crawl" || source === "both") {
      try {
        crawlData = await crawlGoogle(q, location, num);
      } catch (e) {
        errors.crawl = e.message;
        console.error("Crawl err:", e.message);
      }
    }
    ...
    if (!serpData && !crawlData) {
      return res
        .status(500)
        .json({ error: "Cả hai nguồn đều thất bại", details: errors });
    }
    ...
    if (source === "both" && serpData && crawlData) {
      result = {
        ...
        crawl_raw: {
          ads: crawlData.ads,
          organic_results: crawlData.organic_results,
        },
    ...
    try {
      insertSearch.run({
        ...
        total_results:
          (result.organic_results?.length || 0) + (result.ads?.length || 0),
      });
    } catch (e) {
...
app.post("/api/batch", async (req, res) => {
  ...
  for (const kw of keywords.slice(0, 10)) {
    try {
      ...
      const data =
        source === "both" && serpData && crawlData
          ? {
              query: kw,
              source: "both",
              timestamp: new Date().toISOString(),
      ...
      try {
        insertSearch.run({
          ...
          crawl_json: JSON.stringify(crawlData?.organic_results || []),
          total_results:
            (data.organic_results?.length || 0) + (data.ads?.length || 0),
        });
      } catch {}
